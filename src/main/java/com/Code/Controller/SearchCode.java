...
public class SearchCode {
	...
	public ResponseEntity<List<CodeObjectRequest>> getSpecificCode(
			@PathVariable String number, @RequestBody SearchReqObj searchReqObj) {
		if (!userService.checkUserExist(searchReqObj)) {
		...
		try {
			...
			int num = Integer
					.parseInt(number.substring(1, number.length() - 1));
			Pageable range = new PageRequest(num, 5);
			List<Code> userCodes = codeRepository.findByusernameAndsdescription(searchReqObj.getUsername(),
			...
			if (userCodes.size() == 0) {
				...
				List<CodeObjectRequest> codeObjectRequests = codeService.changeFromDomainToRequestObj(userCodes);
				return new ResponseEntity<List<CodeObjectRequest>>(
